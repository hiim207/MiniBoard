pipeline {
    ...
    stages {
        ...
        stage("Docker Image Build") {
           steps {
               sh "docker build -t lee20jin/apache2_smboard:${BUILD_NUMBER} ./docker/apache2/"
               sh "docker build -t lee20jin/smboard_smboard:${BUILD_NUMBER} ./docker/smboard/"
               sh "docker build -t lee20jin/mariadb_smboard:${BUILD_NUMBER} ./docker/mariadb/"
           }
        }
        stage("Docker Image Push") {
           steps {
               sh "docker push lee20jin/apache2_smboard:${BUILD_NUMBER}"
               sh "docker push lee20jin/smboard_smboard:${BUILD_NUMBER}" 
               sh "docker push lee20jin/mariadb_smboard:${BUILD_NUMBER}" 
           } 
        }
        stage("Docker Image Clean up") {
           steps {
               sh "docker image rm lee20jin/apache2_smboard:${BUILD_NUMBER}" 
               sh "docker image rm lee20jin/smboard_smboard:${BUILD_NUMBER}" 
               sh "docker image rm lee20jin/mariadb_smboard:${BUILD_NUMBER}" 
           }
        }
